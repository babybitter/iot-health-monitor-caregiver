...
只有尚未接入接口的任务、证书和培训使用隔离的演示内容。患者身份不虚构，生命体征、输液、设备状态、异常和趋势均来自运行期间实际接收的 MQTT 消息；未收到数据时统一显示空状态或 `--`。
...
│  ├─ certificates/     演示证书与后续接口承载页
│  ├─ training/         演示培训与后续接口承载页
...
- 工作台：值班与签到、实时异常数量、MQTT 状态、当前照护对象、最近生命体征和演示任务摘要
- 智能监控：八项生命与环境指标、真实阈值异常、异常核对、输液重量与滴速、会话趋势、设备状态和 MQTT 控制
- 任务：演示任务的状态筛选、分页、详情、开始、完成和记录入口
...
生命体征已经直接接入 MQTT。任务、证书和培训仍缺少服务端接口，因此暂由 `mocks/demo-business-data.js` 提供演示内容；该文件不包含任何生命体征、设备、异常或趋势数据。后续接入点集中在 `services/repository.js`：
...
页面只调用 repository，不直接引用演示模块；替换为真实接口时无需重写页面。
...
- 护工业务接口尚未提供，任务、证书和培训当前为有明确标识的演示内容；任务操作、本机考勤和照护记录不能跨设备同步。
